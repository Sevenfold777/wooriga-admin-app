...
import { getDAUApi } from "../../api/UserApi";
...
export default function Dau({ navigation, route: { params } }) {
  ...
  const {} = useQuery(["DAU", { prev }], () => getDAUApi({ prev }), {
    onSuccess: ({ data }) => {
      if (data.length === 0) {
        setIsLast(true);
      } else {
  ...
  const renderBar = ({ item }) => (
    <BarItem
      style={{
        height: (item.count / (parseInt(max / pageHeight) + 1)) * 8,
        width: 50,
        alignItems: "center",
      }}
    >
